        ...
        <div class="container-fluid">
          ...
          <div class="row">
            <div class="col-12">
              ...
              <div class="card">
                <div class="card-body">
                  <h5 class="card-title">Invoies</h5>
                  <div class="table-responsive">
                    ...
                    >
                      ...
                      <?php
                      $statement=$connect->prepare("SELECT * FROM invoice i INNER JOIN customers c ON i.customer_id=c.user_id ORDER BY i.invoice_id DESC");
                      $statement->execute();
                      $count=0;
                      foreach($statement->fetchAll() as $row){
                        $total=get_order_total($connect, $row["order_id"]);
                        $paid=get_total_paid($connect, $row["order_id"]);
                        $balance=$total-$paid;
                        $status = ($balance==0) ? '<span  class="badge rounded-pill bg-success text-white">
                          completed
                        </span>' : '<span  class="badge rounded-pill bg-warning text-white">
                          incomplete
                        </span>';
                        echo '
                        <tr>
                          <td>INV'.str_pad($row["invoice_id"], 5, "0", STR_PAD_LEFT).'</td>
                          <td>OD'.str_pad($row["order_id"], 5, "0", STR_PAD_LEFT).'</td>
                          ...
                          <td>  <a href="view-invoice.php?id='.$row["order_id"].'" class="btn btn-primary btn-sm">
                              View
                              <span class="fas fa-eye"></span>
                            </a> |
                            <button id="'.$row["order_id"].'" class="btn btn-dark btn-sm print invoice">
                                  Print
